...
CREATE TABLE `dienstrooster_availability` (
	...
	`person_id` text NOT NULL,
	`slot_id` text NOT NULL,
	`blocking_level` text CHECK(`blocking_level` IN ('ABSOLUUT', 'LIEVER_NIET')),
...
CREATE TABLE `dienstrooster_notification_log` (
	...
	`person_id` text NOT NULL,
	`period_id` text,
	`type` text NOT NULL CHECK(`type` IN ('PERIOD_OPENED', 'PARTTIME_CHECK', 'REMINDER', 'FINAL_WARNING', 'DEADLINE_PASSED', 'BLOCK_OVERRIDDEN', 'SCHEDULE_PUBLISHED', 'SWAP_REQUESTED', 'SWAP_RESULT', 'CORRECTION_BOOKED')),
...
CREATE TABLE `dienstrooster_notification_template` (
	`id` text PRIMARY KEY NOT NULL,
	`sleutel` text NOT NULL CHECK(`sleutel` IN ('PERIOD_OPENED', 'PARTTIME_CHECK', 'REMINDER', 'FINAL_WARNING', 'DEADLINE_PASSED', 'BLOCK_OVERRIDDEN', 'SCHEDULE_PUBLISHED', 'SWAP_REQUESTED', 'SWAP_RESULT', 'CORRECTION_BOOKED')),
	`onderwerp` text NOT NULL,
