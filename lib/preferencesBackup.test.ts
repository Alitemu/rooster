...
function createFixture(): Fixture {
  ...
  const periodNaam = `Backup-test-${periodId.slice(0, 8)}`;
  db.prepare(
...
afterEach(() => {
  ...
  while (createdPeriodIds.length > 0) {
    ...
    db.prepare(
      'DELETE FROM dienstrooster_person WHERE id IN (SELECT person_id FROM dienstrooster_pool_membership WHERE pool_id = ?)'
    ).run(period.pool_id);
    const pool = db
...
describe('writePreferencesBackup', () => {
  ...
  it('replaces the previous backup file instead of leaving it alongside the new one', () => {
    ...
    const prefix = `${fixture.codenaam}__${fixture.periodNaam.replace(/[^a-zA-Z0-9_-]+/g, '_')}__`;
