...
/**
 * Window rule - manual assignment paths.
 *
 * The solver enforces "at most one shift per person per windowWeeks" as a
 * hard constraint, but manual gap-filling and reassignment go straight to
 * the database and never call the solver. This is the one rule/one test
 * proof that the same restriction holds there too - including across a
 * year boundary, where a bare iso_week subtraction silently breaks down
 * (week 52 of one year and week 1 of the next are one calendar week
 * apart, not fifty-one).
 */
...
describe('personWouldViolateWindowRule', () => {
  ...
  it('catches the violation across a year boundary', () => {
